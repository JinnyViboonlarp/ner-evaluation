...
def evaluate_whole_folder(goldpath, testpath, resultpath):
    ...
    result = {}
    for mode in ['strict','token']:
        result[mode] = classification_report(y_true_all[mode], y_pred_all[mode], mode='strict', output_dict=False)
...
if __name__ == "__main__":
    ...
    if(os.path.isdir(args.goldpath) and os.path.isdir(args.testpath)):
        ...
        either both of them are directories, or both are regular files")
...
#python evaluate.py goldfiles/ testfiles/
#python evaluate.py goldfiles/cpb-this-is-example1-transcripts.ann testfiles/spacy-cpb-this-is-example1-transcripts.mmif result-one-pair.txt
